	...
	<view>
		...
		<scroll-view id="scroll" scroll-y = "true" :style="{height:scrollHeight}">
			<view class="cu-list menu card-menu margin-top-sm">
				<navigator v-for="(item,i) in list" :key="i" class="cu-item" :url="'../detail/detail?id=' + item.id">
					<view class="content padding-tb-sm">
						<view>产品名称：{{item.customerid}}</view>
						<view class="text-gray ">
							 <template v-if="item.state==1">
							 入库数量：{{item.customerphone}}
							 </template>
							 <template v-else>
							 出库数量：{{item.customerphone}} 
							 </template>
						</view>
						<view class="text-gray ">
							 操作时间：{{item.customername}}
						</view>
					</view>
					<view class="action">
						<template v-if="item.state==1">
