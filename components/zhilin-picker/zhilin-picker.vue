		...
		<uni-popup ref="popup" type="top" @change="popupChange">
			...
			<view class="main" v-if="list.length">
				<scroll-view scroll-y="true" @scrolltolower="lower" :lower-threshold="1">
					<view class="scroll-view-item" v-for="(v,i) in list" :key="i" @tap="tapItem(v.value,v.label)" :class="selected.indexOf(v.value)>-1?'selected':null">
						<text>{{v.label}}</text>
	...
	export default {
		...
		methods: {
			...
			tapItem(val,label) {
				if (this.multiple) {
					...
					if (idx == -1) {
						...
						this.sellabel.push(label)
					} else {
						this.selected.splice(idx, 1)
						this.sellabel.splice(idx, 1)
					}
				} else {
					this.selected = [val]
					this.sellabel = [label]
			...
			tapOK() {
				...
				this.$emit("confirmlabel",this.sellabel.join())
			},
			popupChange(e) {
